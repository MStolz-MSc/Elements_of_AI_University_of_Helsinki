# -*- coding: utf-8 -*-
"""
Created on Wed Dec 23 13:06:10 2020
...
N = 1000     # size of the problem is N x N                                      
...
def main():
    ...
    for step in range(steps):
        ...
        for i in range(tracks):
            ...
            prob = np.exp(-(S_old - S_new)/T) # Simulating Annealing 
            
            if S_new > S_old:
                x[i], y[i] = x_new, y_new   # new solution is better, go there
                
            elif T>0 and random.random()<prob: # choose worse solution by chance
                x[i], y[i] = x_new, y_new
